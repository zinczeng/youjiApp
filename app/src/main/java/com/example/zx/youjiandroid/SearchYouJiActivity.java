...
public class SearchYouJiActivity extends Activity implements View.OnClickListener {
    private FlowLayout mFlowlayout;
    private ImageView searchback_to_youji;
    private AsyncTaskHelper.OnDataDownloadListener downloadListener;
    private AsyncTaskHelper taskHelper;
    private RecyclerView tag_yj;
    private TextView select_yj;
    private EditText yj;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_search_youji);
        mFlowlayout = (FlowLayout)findViewById(R.id.Flow_biaoqian);
        tag_yj = (RecyclerView) findViewById(R.id.tag_youji);
        select_yj = (TextView) findViewById(R.id.select_yj);
        select_yj.setOnClickListener(this);
        yj = (EditText) findViewById(R.id.yj);
        initData();
        init();
    }

    private void init() {
        searchback_to_youji=(ImageView) findViewById(R.id.searchback_to_youji);
        searchback_to_youji.setOnClickListener(this);
    }
            ...
            public void onDataDownload(byte[] result) {
                ...
                if (code.equals("1")) {
                   ...
                   tag = FastJsonTools.getBeans(Result,Tag.class);
                    LayoutInflater inflater = LayoutInflater.from(SearchYouJiActivity.this);
                    for(int i=0;i<tag.size();i++){
                        TextView tv =(TextView) inflater.inflate(R.layout.layout,mFlowlayout,false);
    ...
    public void onClick(View view) {
        switch (view.getId()){
            case R.id.searchback_to_youji:
                ...
                break;
            case R.id.select_yj:
                if(!yj.getText().toString().equals("")){
                    selectgl( yj.getText().toString());
                }else {
                    Toast.makeText(SearchYouJiActivity.this,"输入内容不能为空",Toast.LENGTH_SHORT).show();
                }
                break;
        }
    }
        ...
        downloadListener = new AsyncTaskHelper.OnDataDownloadListener() {
            ...
            public void onDataDownload(byte[] result) {
                ...
                if (code.equals("1")) {
                    ...
                    Toast.makeText(SearchYouJiActivity.this,"暂未查询到游记",Toast.LENGTH_SHORT).show();
                }
    ...
    private void recyclerView() {
        tag_yj.setLayoutManager(new LinearLayoutManager(getApplicationContext(), LinearLayoutManager.VERTICAL,false));
        RecycleItemClickListener itemClickListener = new RecycleItemClickListener() {
            @Override
            public void onItemClick(View view, int position) {
                Intent intent=new Intent(SearchYouJiActivity.this,ProductDetailActivity.class);
